...
public class WidgetRefresher {
	...
	private WidgetUpdateData createWidgetUpdateData(RawAirvoiceData rawData,
        ...
        if(hasAmountIncreased(context, rawData.getPhoneNumber(), amount)){
            notifyOfAmountIncrease(context, appWidgetId, rawData, warningLimit,
                    displayType);
        }
    ...
    private boolean hasAmountIncreased(Context context, String phoneNumber, float currentAmount) {
        AmountDbAdapter amountDbAdapter = new AmountDbAdapter(context);
        amountDbAdapter.open();
        RawAirvoiceData latestRawAirvoiceData = amountDbAdapter.getLatestRawAirvoiceData(phoneNumber);
        amountDbAdapter.close();

        if (latestRawAirvoiceData == null){
            return false;
        } else{
            return (latestRawAirvoiceData.getDollarValue() + epsilon) < currentAmount;
        }
    }
    ...
    private void notifyOfAmountIncrease(Context context, int appWidgetId,
                                        RawAirvoiceData rawData, int warningLimit, String displayType) {
        String name = sharedStorage.getNameLabel(context, appWidgetId);
        Plan plan = planFactory.createPlan(rawData.getPlanText());
        if (plan != null) {
            String message = name + " has been refilled. ("
                    + plan.getTextForWidget(warningLimit, displayType) + ")";
            String title = "Refilled";
