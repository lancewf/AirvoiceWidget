...
public class WidgetRefresher {
	...
	public void updateWidgetFromCache(Context context, int appWidgetId){
		if (cacheStorage.isThereCachedData(context, appWidgetId)) {
			try {
				String rawDataString = cacheStorage.getRawCachedValue(context,
						appWidgetId);
				updateWidget(rawDataString, context, appWidgetId);
			} catch (Exception ex) {
	...
	public void updateWidget(String rawDataString, 
			Context context, int appWidgetId){
		if (rawDataString != null) {
			try {
				DataParser dataParser = new DataParser();
				RawAirvoiceData rawData = dataParser.parseRawData(rawDataString);
				if (rawData == null && cacheStorage.isThereCachedData(context, appWidgetId)) {
					String cacheRawDataString = cacheStorage.getRawCachedValue(
							context, appWidgetId);
					rawData = dataParser.parseRawData(cacheRawDataString);
				}
				if (rawData != null) {
					cacheStorage.saveCacheData(context, appWidgetId, rawDataString);
					WidgetUpdateData widgetUpdateData = createWidgetUpdateData(
							rawData, context, appWidgetId);
	...
	private WidgetUpdateData createWidgetUpdateData(RawAirvoiceData rawData, 
		...
		widgetUpdateData.valueText = rawData.plan.getTextForWidget(amount, displayType);
		...
		if(daysToExpire > warningDays){
			...
			sharedStorage.clearSentWarningExpireNotification(context, appWidgetId);
		} else{
		...
		if (amount < warningLimit) {
			notifyWarningAmount(context, appWidgetId, rawData, warningLimit, displayType);
			...
			sharedStorage.clearSentWarningAmountNotification(context, appWidgetId);
	...
	private Boolean isDataOld(final Context context, final int appWidgetId){
		if(cacheStorage.isThereCachedData(context, appWidgetId)){
			Calendar lastUpdate = cacheStorage.getLastUpdatedDate(context, appWidgetId);
	...
	private void notifyWarningAmount(Context context, int appWidgetId, 
		...
		if (!sharedStorage.hasSentWarningAmountNotification(context, appWidgetId)) {
			String name = sharedStorage.getNameLabel(context, appWidgetId);
			String message = name + " is below "
					+ rawData.plan.getTextForWidget((double) warningLimit,
							displayType);
			String title = "Below Limit";
			sendNotification(title, message, context);
			sharedStorage.setSentWarningAmountNotification(context, appWidgetId);
		}
	}
	
	private void notifyWarningExpire(Context context, int appWidgetId, int daysWarningExpire){
		if (!sharedStorage.hasSentWarningExpireNotification(context, appWidgetId)) {
			String name = sharedStorage.getNameLabel(context, appWidgetId);
			String message = name + " expires in less than " + daysWarningExpire + " days";
			String title = "Expiration Warning";
			sendNotification(title, message, context);
			sharedStorage.setSentWarningExpireNotification(context, appWidgetId);
		}
	}
